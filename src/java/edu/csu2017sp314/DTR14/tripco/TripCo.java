...
public class TripCo{
	...
	public TripCo(String _xml, String _svg, String fileroot, boolean[] opts){
		this._xml = _xml;
		this._svg = _svg;
		this._gui = opts[0];
		this._id = opts[1];
	...
	public void initiate() throws FileNotFoundException, Exception{
		...
		Presenter present = new Presenter(files, _xml, _svg, fileroot, opt);
	...
	public static void main(String[] args) throws FileNotFoundException, InterruptedException, Exception{
		...
		TripCo tc = new TripCo(_xml, _svg, "", opts);
                tc.addFile(files.get(0));
		tc.initiate();
	}
}